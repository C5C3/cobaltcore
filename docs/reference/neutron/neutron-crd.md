...
| `targetCPUUtilization must be at least 1` | The target is below 1. There is no upper bound, as in `autoscaling/v2` |
| `targetMemoryUtilization must be at least 1` | The target is below 1. There is no upper bound, as in `autoscaling/v2` |
| `%s %d can never be reached: no container of the API pod can use more than %d%% of its %s request, because its %s limit caps it there; set a %s limit above the request, or a target of at most %d` | A target above 100 that the limits of `spec.deployment.resources` make unreachable, reported at the target. A block that names no memory renders the same figure as request and limit, so a memory target above 100 needs a memory limit above the request or a request without a limit |
| `stabilizationWindowSeconds must be between 0 and 3600` | `spec.autoscaling.behavior.<scaleUp\|scaleDown>.stabilizationWindowSeconds` is outside the `autoscaling/v2` range |
| `Unsupported value` (`selectPolicy`, policy `type`) | A `selectPolicy` other than `Max`, `Min` or `Disabled`, or a policy `type` other than `Pods` or `Percent` |
| `value must be greater than zero` | A behavior policy `value` of zero or below |
| `periodSeconds must be between 1 and 1800` | A behavior policy `periodSeconds` outside the `autoscaling/v2` range |
| `tolerance must not be negative` | A negative behavior `tolerance` |
