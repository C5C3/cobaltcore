...
| 3 | Create the missing ExternalSecrets | `apply` | Applies `01-late-secrets.yaml` — creates ExternalSecrets pointing to existing OpenBao paths (the per-ControlPlane `openstack/keystone/openstack/controlplane/db` and per-CR `bootstrap/openstack/controlplane-keystone/admin`) |
