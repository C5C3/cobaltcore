...
| [e2e-autoscaling](#e2e-autoscaling) | `cp-autoscaling` | The API autoscaler with metrics-server: token load scales Keystone from one pod to its HPA maximum and back within 240 s, the budget admits an eviction at the minimum, and every database-backed API fits its SQL connection cap at its maximum |
...
| invalid-sizingprofile-cr | multiple (rejected, cluster-scoped) | Every `SizingProfile` marker and webhook rejection path pinned to a deterministic admission failure (fixtures `00` to `07`). Generated by its `_generate.py` and guarded by `make verify-invalid-cr-fixtures` |
...
`tests/e2e-autoscaling/` proves the API HorizontalPodAutoscalers end to end on
a full ControlPlane with metrics-server. It runs as chainsaw Test
`c5c3-autoscaling` in its own CI job, `e2e-autoscaling`, on a kind cluster of
its own, and lives outside `tests/e2e/` for the reason
[federated-controlplane](#federated-controlplane) gives.

The ControlPlane `cp-autoscaling` carries the seven database-backed services of
`full-controlplane-keystone` (no Horizon) on the `Minimal` profile, with two
changes that make the caps testable:

- `infrastructure.database.credentialsMode: Static`. The operators size each
  SQL user's `max_user_connections` from `autoscaling.maxReplicas`, but on the
  default `Dynamic` mode they create no MariaDB `User` CR and the OpenBao role
  sets no cap, so only a Static user carries one. Nothing seeds the Static KV
  paths, so the suite writes `kv-v2/openstack/<service>/openstack/cp-autoscaling/db`
  for each service (and `…/nova/…/api-db`) before it applies the plane.
- `spec.sizing`: Keystone runs two uWSGI processes and autoscales from one pod
  to three on `targetCPUUtilization: 150`, with a `scaleDown` behavior of a
  15 s window and a `Percent 100 / 15 s` policy. Glance, Placement, Barbican,
  Neutron, Cinder and Nova run `minReplicas = maxReplicas = 2`, which the HPA
  controller enforces before it reads any metric, so each is checked at its
  maximum without a load of its own.

| Step | Behaviour Validated |
| --- | --- |
| 1. `preflight` (3m) | metrics-server serves `/apis/metrics.k8s.io/v1beta1/nodes`, and the `controlplanes.c5c3.io` CRD exists |
...
| 5. `start-load` (3m) | The load Job's pod runs |
| 6. `scale-out` (8m) | The Keystone HPA reaches `desiredReplicas: 3` and the Deployment three ready pods; the peak `averageUtilization` seen is above 150 |
| 7. `keystone-at-maximum` (5m) | `check-connection-cap.sh` for Keystone at three pods, retried for up to 240 s while the load reaches the pods the HPA just added |
| 8. `service-burst` (8m) | The burst Job completes with six `RESULT` lines at `fail=0` |
| 9. `pinned-at-maximum` (10m) | `check-connection-cap.sh` for the six pinned APIs (Nova with its API and cell users), and the server-wide connection count stays below `@@max_connections` |
| 10. `load-result-and-scale-in` (20m) | The load Job completes with `fail=0` and at least 300 requests, and Keystone is back at one ready pod within 240 s, which the default 300 s window would not allow. Completed CronJob pods that match the Keystone scale target are removed while it polls (see below) |
| 11. `pdb-at-minimum` (8m) | Keystone's PDB allows one disruption, an Eviction of the remaining pod is admitted, and the Deployment and the Keystone child return to one ready pod and `Ready` |
| 12. `behavior-removal` (5m) | A JSON patch removes the Keystone behavior; the child carries none and the HPA's scale-down window reads 300 (or `spec.behavior` is absent) |

A test-level `catch` (5m) dumps the HPAs, `kubectl top pods`, pods, events,
both Jobs' logs and the live MariaDB connections per user. The `cleanup` of
step 2 (15m) runs when the test ends, whichever step it ends in, and deletes
both Jobs, the ControlPlane, the OVNCentral, the broker vhost and the eight KV
paths. The worst-case ceiling is the sum of these timeouts, 145 minutes.
...
`app.kubernetes.io/name` and `app.kubernetes.io/instance` labels alone, and the
...
minimum. Steps 4 and 10 therefore delete the Succeeded Job pods of the Keystone
...
`v3applicationcredential`. `02-keystone-loadgen-job.yaml` POSTs
`/v3/auth/tokens` from four threads for 900 s, each request on a fresh
connection, and counts only HTTP 201 as ok. `03-service-burst-job.yaml` issues
one token, reads the internal catalog endpoints, exits 2 with
`catalog has no internal <type> endpoint` when one is missing, and sends 60
GETs at a concurrency of 6 to each API. Only HTTP 200 counts: a 401 or 403 is
answered before the service reaches its database.

**`check-connection-cap.sh <child-cr> <sql-user>...`** exits 1 with a named
reason on the first failure:

1. exactly one HPA carries `app.kubernetes.io/instance=<child-cr>`;
2. its scale-target Deployment has as many ready replicas as the HPA maximum;
3. every pod of the Deployment has all containers ready (restart counts are
   printed);
4. every pod holds an ESTABLISHED TCP connection to port 3306, read from
   `/proc/net/tcp` and `/proc/net/tcp6` inside the pod;
5. each SQL user's `User` CR carries a positive `maxUserConnections`, MariaDB's
   `mysql.user.max_user_connections` equals it, and the user's
   `PROCESSLIST` count stays within it;
6. no pod of the child logged `max_user_connections` or `(1226,`, in its
   current or previous container.

**`mariadb-sql.sh <statement>`** runs one statement as root on
`openstack-db-0` and prints the rows without column names. It reads the root
password from the Secret the MariaDB CR's `spec.rootPasswordSecretKeyRef`
names and hands it to the client over stdin as `MYSQL_PWD`, so the password
is on no command line. `check-connection-cap.sh`, step 9 and the `catch` run
their SQL through it.

**`keystone-hpa.sh hpa|deploy|drop-job-pods`** prints the name of the Keystone
child's HPA or of its scale-target Deployment, or deletes the child's
Succeeded Job pods.

Run it locally against a cluster with the stack, K-ORC and the ten operators
deployed:

```bash
WITH_METRICS_SERVER=true WITH_CONTROLPLANE=true CONTROLPLANE_OPERATORS=external \
  CONTROLPLANE_NAME=cp-autoscaling WITH_NFS=true WITH_MESSAGING=true make deploy-infra
make e2e-autoscaling
```
