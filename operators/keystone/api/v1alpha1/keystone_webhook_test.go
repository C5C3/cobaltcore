...
func TestValidate_DeploymentNodePlacementRejected(t *testing.T) {
	for _, tc := range []struct {
	...
	}{
		{
			...
			mutate: func(k *Keystone) { k.Spec.Deployment.NodeSelector = map[string]string{"bad key": "x"} },
			want:   "spec.deployment.nodeSelector: Invalid value",
		},
		{
			name: "toleration without key or Exists",
			mutate: func(k *Keystone) {
				k.Spec.Deployment.Tolerations = []corev1.Toleration{{Operator: corev1.TolerationOpEqual}}
			},
			want: "spec.deployment.tolerations[0].operator: Invalid value",
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
			g := NewGomegaWithT(t)
...
func TestValidate_JobsRejected(t *testing.T) {
	for _, tc := range []struct {
		name string
...
func TestValidateUpdate_FinalizerRemovalOnADeletingCRSkipsValidation(t *testing.T) {
	g := NewGomegaWithT(t)
	...
	stale.Finalizers = []string{"keystone.openstack.c5c3.io/finalizer"}
	stale.DeletionTimestamp = ptr.To(metav1.Now())

	released := stale.DeepCopy()
	released.Finalizers = nil
	g.Expect(w.Default(ctx, released)).To(Succeed())
	...
	g.Expect(err).To(MatchError(ContainSubstring("spec.jobs.priorityClassName: Not found")),
		"a spec edit on a deleting CR is validated like any other")
}

func TestValidate_EmptyJobsAccepted(t *testing.T) {
	g := NewGomegaWithT(t)
	w := &KeystoneWebhook{Client: newFakeClient().Build()}
	k := validKeystone()
	k.Name = "my-ks"
	k.Spec.Jobs = &commonv1.JobSpec{}

	_, err := w.ValidateCreate(context.Background(), k)
	g.Expect(err).NotTo(HaveOccurred())
}

func TestValidate_FederationProxyResourcesRequestAboveLimitRejected(t *testing.T) {
	g := NewGomegaWithT(t)
	w := &KeystoneWebhook{}
	k := validKeystone()
	k.Name = "my-ks"
	k.Spec.Federation = &FederationSpec{ProxyResources: &corev1.ResourceRequirements{
	...
	_, err := w.ValidateCreate(context.Background(), k)
	g.Expect(err).To(HaveOccurred())
	g.Expect(err.Error()).To(ContainSubstring("spec.federation.proxyResources.requests.cpu: Invalid value"))
}
...
// TestKeystoneValidateUpdate_TargetClusterRefAddedRejected covers the presence
...
func TestKeystoneValidateUpdate_TargetClusterRefAddedRejected(t *testing.T) {
	g := NewGomegaWithT(t)
	w := &KeystoneWebhook{}
	old := validKeystone()
	updated := validKeystone()
	updated.Spec.TargetClusterRef = &commonv1.TargetClusterRefSpec{Name: "edge-1"}
...
// TestKeystoneValidateUpdate_TargetClusterRefRemovedRejected covers the presence
// flip downwards: dropping the ref would strand the children on the cluster it
// named.
func TestKeystoneValidateUpdate_TargetClusterRefRemovedRejected(t *testing.T) {
	g := NewGomegaWithT(t)
	w := &KeystoneWebhook{}
	old := validKeystone()
...
// TestKeystoneValidateUpdate_TargetClusterRefChangedRejected covers a rename,
// which would re-point the reconciler at a cluster that holds none of the
// children.
func TestKeystoneValidateUpdate_TargetClusterRefChangedRejected(t *testing.T) {
	g := NewGomegaWithT(t)
	w := &KeystoneWebhook{}
	old := validKeystone()
	old.Spec.TargetClusterRef = &commonv1.TargetClusterRefSpec{Name: "edge-1"}
	updated := validKeystone()
	updated.Spec.TargetClusterRef = &commonv1.TargetClusterRefSpec{Name: "edge-2"}
...
// TestKeystoneValidateUpdate_TargetClusterRefUnchangedAccepted proves the check
// freezes only the ref: an unrelated edit on a CR that names a target cluster
// still passes.
func TestKeystoneValidateUpdate_TargetClusterRefUnchangedAccepted(t *testing.T) {
	g := NewGomegaWithT(t)
	w := &KeystoneWebhook{}
	old := validKeystone()
	old.Spec.TargetClusterRef = &commonv1.TargetClusterRefSpec{Name: "edge-1"}
	updated := old.DeepCopy()
	updated.Spec.Deployment.Replicas = 2

	_, err := w.ValidateUpdate(context.Background(), old, updated)
	g.Expect(err).NotTo(HaveOccurred())
}

// TestKeystoneValidateCreate_EmptyTargetClusterRefNameRejected is the
// defense-in-depth twin of the MinLength marker: a present ref must name a
// cluster.
func TestKeystoneValidateCreate_EmptyTargetClusterRefNameRejected(t *testing.T) {
	g := NewGomegaWithT(t)
	w := &KeystoneWebhook{}
	k := validKeystone()
	k.Spec.TargetClusterRef = &commonv1.TargetClusterRefSpec{Name: ""}
