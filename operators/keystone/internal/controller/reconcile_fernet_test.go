...
// TestFernetRotationCronJob_PodSecurityContextSetsFSGroup verifies that the
// rotation Pod template carries SecurityContext.FSGroup = deployment.OpenStackUID so the
...
// the read-only Secret-backed key volumes mounted into the rotation Pod use
