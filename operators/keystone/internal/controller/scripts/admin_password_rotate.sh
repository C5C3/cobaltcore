...
# Unlike fernet_rotate.sh / credential_rotate.sh there is no
...
# commits it onto the operator-owned push-source Secret (split-compute-write). Only Python standard-library modules are used to avoid image
# dependencies.
# NOTE: The Python K8s API PATCH block below MUST stay in sync with
# fernet_rotate.sh / credential_rotate.sh.
python3 << 'PYTHON'
...
# check in validateAdminPasswordRotationOutput.
