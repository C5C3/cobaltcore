...
// against the metrics contract exposed by the Neutron operator.
//
...
import (
	...
	"github.com/c5c3/cobaltcore/operators/neutron/internal/metrics"
)
...
// directory.
const dashboardPath = "neutron-operator.json"
...
// matches down to those starting with `neutron_operator_` so we ignore
...
func TestDashboardParsesAsJSON(t *testing.T) {
	...
	g.Expect(d.UID).To(Equal("neutron-operator"),
	...
	// The db-sync panel surfaces the per-CR neutron_operator_db_sync_*
	...
	// The ovn-db-sync twin covers the recurring neutron-ovn-db-sync-util
	// CronJob, which no sibling operator runs.
	g.Expect(titles).To(HaveKey("ovn_db_sync duration p95 and failure rate"),
		"dashboard must include the ovn-db-sync duration/failure panel")

	// The runs-per-result panel is the only view that puts the two job
	// families side by side, so a schema migration that succeeds while the
	// OVN synchronisation fails is visible in one place.
	g.Expect(titles).To(HaveKey("db_sync and ovn_db_sync runs per result"),
		"dashboard must include the combined runs-per-result panel")
...
// drift: every `neutron_operator_*` identifier that appears in a panel
...
func TestDashboardReferencesOnlyRegisteredMetrics(t *testing.T) {
	...
	// this test binary is the only neutron_operator registrant, so the
	// mirrored instance yields the exact metric families production emits.
	const probe = "dashboard_test_probe"
	subReconcilerMetrics := instrumentation.NewMetrics("neutron_operator")
	...
	metrics.RecordOVNDBSync(probe, probe, "succeeded", 0)
	...
	for _, fam := range families {
		name := fam.GetName()
		if strings.HasPrefix(name, "neutron_operator_") {
			registered[name] = struct{}{}
		}
	}
	// Clean up the probe series so subsequent tests in the same binary
	// see a clean per-CR registry.
	metrics.DeleteForNeutron(probe, probe)
	g.Expect(registered).NotTo(BeEmpty(),
		"expected at least one neutron_operator_* metric to be registered")
	...
	for _, p := range d.Panels {
		for _, tr := range p.Targets {
			for _, tok := range metricNameRE.FindAllString(tr.Expr, -1) {
				if !strings.HasPrefix(tok, "neutron_operator_") {
					continue
				}
	...
	g.Expect(seen).NotTo(BeEmpty(),
		"dashboard must reference at least one neutron_operator_* metric")
