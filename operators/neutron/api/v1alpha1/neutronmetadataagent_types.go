...
type NeutronMetadataAgentSpec struct {
	...
	// pod, per resource: a CPU the block names neither as request nor as limit
	// gets a 100m request and no limit, and a memory the block names neither
	// way gets 368Mi as both request and limit. A resource the block names is
	// used as written, and anything else it sets is kept.
	// +optional
