...
// Tests for the Cinder operator entrypoint wiring. They assert against the
// package-level scheme this binary hands to bootstrap.Run, without standing up a
// cluster or envtest.
package main
...
// against a scheme they build themselves, so a kind added to the list without its
...
func TestRemoteChildKindsAreRegisteredInTheScheme(t *testing.T) {
	for _, gvk := range controller.CinderRemoteChildKinds {
