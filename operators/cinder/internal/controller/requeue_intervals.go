...
// Cinder-specific requeue intervals that no other operator shares.
const (
	// RequeueDatabaseWait is the interval for waiting on MariaDB CR readiness and
	// db-sync Job completion before the database schema can be provisioned. These
	// operations are moderately slow, so a longer interval avoids unnecessary API
	// churn. It mirrors glance's RequeueDatabaseWait.
	...
	// It mirrors glance's RequeueUpgradeWait.
