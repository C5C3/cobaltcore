...
func TestReconcileSecrets_CredentialGates(t *testing.T) {
	dbSecretWrongKeys := &corev1.Secret{
		...
		Data:       map[string][]byte{"username": []byte("barbican")}, // password missing
	}
	...
	tests := []struct {
	...
	}{
		{
			...
			secrets:    []client.Object{barbicanServiceUserSecret("svc-pw")},
			wantReason: "WaitingForDBCredentials",
		},
		{
			name:       "database Secret missing the password key",
			secrets:    []client.Object{dbSecretWrongKeys, barbicanServiceUserSecret("svc-pw")},
			wantReason: "WaitingForDBCredentials",
		},
		{
			name:       "service-user Secret absent",
			secrets:    []client.Object{barbicanDBSecret()},
			wantReason: "WaitingForServiceUserCredentials",
		},
		{
			name:       "service-user Secret missing the password key",
			secrets:    []client.Object{barbicanDBSecret(), serviceUserWrongKey},
			wantReason: "WaitingForServiceUserCredentials",
		},
	}
	for _, tc := range tests {
...
// TestReconcileSecrets_CustomServiceUserKeyIsGated covers a CR whose
// serviceUser.secretRef.key names something other than the webhook default: the
// gate and the digest read must both follow the configured key.
func TestReconcileSecrets_CustomServiceUserKeyIsGated(t *testing.T) {
	g := NewGomegaWithT(t)
	barbican := testBarbican()
	barbican.Spec.ServiceUser.SecretRef.Key = "svc-password"
	serviceUser := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{Name: "barbican-service-user", Namespace: testNamespace},
		Data:       map[string][]byte{"svc-password": []byte("custom-pw")},
	}
	r := newBarbicanTestReconciler(barbican,
	...
	g.Expect(err).NotTo(HaveOccurred())
	g.Expect(res.IsZero()).To(BeTrue())
	g.Expect(digest).To(Equal(secrets.AdminPasswordDigest("custom-pw")))
}
...
func TestReconcileSecrets_ReadErrorPropagatesWrapped(t *testing.T) {
	...
	serviceUserKey := client.ObjectKey{Namespace: testNamespace, Name: "barbican-service-user"}
