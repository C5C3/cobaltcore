...
func TestBarbicanValidate_NodePlacementRejected(t *testing.T) {
	for _, block := range []struct {
		name       string
		deployment func(o *Barbican) *commonv1.DeploymentSpec
		path       string
	}{
		{name: "spec.deployment", deployment: func(o *Barbican) *commonv1.DeploymentSpec { return &o.Spec.Deployment }, path: "spec.deployment"},
	} {
		for _, tc := range []struct {
			...
			t.Run(block.name+"/"+tc.name, func(t *testing.T) {
				...
				_, err := (&BarbicanWebhook{}).ValidateCreate(context.Background(), o)
				g.Expect(err).To(gomega.HaveOccurred())
...
func TestBarbicanValidate_JobsRejected(t *testing.T) {
	for _, tc := range []struct {
		name string
		...
		t.Run(tc.name, func(t *testing.T) {
			g := gomega.NewWithT(t)
			w := &BarbicanWebhook{Client: fake.NewClientBuilder().WithScheme(clientgoscheme.Scheme).Build()}
			o := validBarbican()
			o.Spec.Jobs = tc.jobs
...
func TestBarbicanValidateUpdate_FinalizerRemovalOnADeletingCRSkipsValidation(t *testing.T) {
	g := gomega.NewWithT(t)
	...
	stale.Finalizers = []string{"barbican.openstack.c5c3.io/finalizer"}
...
func TestBarbicanValidate_EmptyJobsAccepted(t *testing.T) {
	g := gomega.NewWithT(t)
	w := &BarbicanWebhook{Client: fake.NewClientBuilder().WithScheme(clientgoscheme.Scheme).Build()}
	o := validBarbican()
...
// TestBarbicanValidate_AutoscalingTargetsAndBehavior pins the target and behavior
...
func TestBarbicanValidate_AutoscalingTargetsAndBehavior(t *testing.T) {
	g := gomega.NewWithT(t)
	w := &BarbicanWebhook{}
	with := func(a *AutoscalingSpec) *Barbican {
		o := validBarbican()
