...
func (w *BarbicanWebhook) validate(ctx context.Context, b *Barbican, extra field.ErrorList) error {
	...
	if b.Spec.Autoscaling != nil {
		...
		// rejected below.
		if b.Spec.Autoscaling.TargetCPUUtilization != nil && *b.Spec.Autoscaling.TargetCPUUtilization < 1 {
			allErrs = append(allErrs, field.Invalid(
		...
		if b.Spec.Autoscaling.TargetMemoryUtilization != nil && *b.Spec.Autoscaling.TargetMemoryUtilization < 1 {
			allErrs = append(allErrs, field.Invalid(
	...
	allErrs = append(allErrs, validation.AutoscalingTargetRequests(specPath.Child("deployment", "resources"), b.Spec.Deployment.Resources, b.Spec.Autoscaling)...)
	...
	// is rejected.
	allErrs = append(allErrs, validation.AutoscalingTargetsReachable(specPath.Child("autoscaling"), b.Spec.Autoscaling, b.Spec.Deployment.Resources)...)
