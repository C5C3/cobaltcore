...
// Package main is the entrypoint for the Placement operator.
//
	...
	if err := bootstrap.Run(bootstrap.ManagerConfig{
		...
		SetupFunc: func(mcMgr mcmanager.Manager, webhooks bool, maxConcurrentReconciles int) error {
			...
			if webhooks {
				...
				if err := (&placementv1alpha1.PlacementWebhook{Client: mgr.GetAPIReader()}).SetupWebhookWithManager(mgr); err != nil {
					return err
				}
			}
			return nil
