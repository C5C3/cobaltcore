...
// against the metrics contract exposed by the Placement operator.
//
...
import (
	...
	"github.com/c5c3/cobaltcore/operators/placement/internal/metrics"
...
// directory.
const dashboardPath = "placement-operator.json"
...
// matches down to those starting with `placement_operator_` so we ignore
...
func TestDashboardParsesAsJSON(t *testing.T) {
	...
	g.Expect(d.UID).To(Equal("placement-operator"),
	...
	g.Expect(len(d.Panels)).To(BeNumerically(">=", 4),
		"dashboard must contain the four panels of the operator dashboard set")
	...
	// The db-sync panel surfaces the per-CR placement_operator_db_sync_*
...
// drift: every `placement_operator_*` identifier that appears in a panel
...
func TestDashboardReferencesOnlyRegisteredMetrics(t *testing.T) {
	...
	// this test binary is the only placement_operator registrant, so the
	// mirrored instance yields the exact metric families production emits.
	const probe = "dashboard_test_probe"
	subReconcilerMetrics := instrumentation.NewMetrics("placement_operator")
	...
	for _, fam := range families {
		name := fam.GetName()
		if strings.HasPrefix(name, "placement_operator_") {
			registered[name] = struct{}{}
		}
	}
	// Clean up the probe series so subsequent tests in the same binary
	// see a clean per-CR registry.
	metrics.DeleteForPlacement(probe, probe)
	g.Expect(registered).NotTo(BeEmpty(),
		"expected at least one placement_operator_* metric to be registered")
	...
	for _, p := range d.Panels {
		for _, tr := range p.Targets {
			for _, tok := range metricNameRE.FindAllString(tr.Expr, -1) {
				if !strings.HasPrefix(tok, "placement_operator_") {
					continue
				}
	...
	g.Expect(seen).NotTo(BeEmpty(),
		"dashboard must reference at least one placement_operator_* metric")
