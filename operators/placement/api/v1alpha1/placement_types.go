...
// that Placement CRs without it continue to work with hardcoded defaults in the
...
type UWSGISpec struct {
	...
	// The default literal mirrors DefaultUWSGIProcesses in placement_webhook.go.
	// +kubebuilder:validation:Minimum=1
	// +kubebuilder:default=2
	Processes int32 `json:"processes,omitempty"`

	// Threads is the number of threads per uWSGI worker process.
	// The default literal mirrors DefaultUWSGIThreads in placement_webhook.go.
	// +kubebuilder:validation:Minimum=1
	...
	// documented default (true, DefaultUWSGIHTTPKeepAlive in
	// placement_webhook.go) when the pointer is nil, and the reconciler falls
	// back to the same default for CRs that bypass the webhook. An explicit false
	// is honored verbatim.
	// +optional
