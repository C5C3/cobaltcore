...
type PlacementSpec struct {
	...
	// db-expand, db-migrate and db-contract upgrade phases. A field left unset
	// falls back to spec.deployment: the priority class, the node selector,
	// the tolerations, and the node affinity (never the pod (anti-)affinity).
	// An empty value opts out of the fallback. Unset resources default to a
	// 100m CPU request and 368Mi memory as request and limit.
	// +optional
