...
func withTestInstrumenter(t *testing.T) *prometheus.Registry {
	...
	m := instrumentation.NewMetricsOnRegistry("glance_operator", reg)
...
// only verifies the glance wiring (the map and the glance_operator metrics
// prefix).
func TestInstrumenterInstrument_RecordsMetrics(t *testing.T) {
	...
	m := findMetricByLabels(t, reg, "glance_operator_reconcile_duration_seconds", durLabels)
	...
	m = findMetricByLabels(t, reg, "glance_operator_reconcile_errors_total", errLabels)
	g.Expect(m).NotTo(BeNil())
