...
// aliases keep the package-local names the reconcilers reference.
const (
	...
	// readiness. The GlanceBackend controller uses it as the liveness backstop
	// while its CredentialsReady / ConfigProjected gates wait.
	RequeueSecretPolling = commonreconcile.RequeueSecretPolling

	// RequeueHealthCheck is the interval for requeuing when the Glance API
	// health check fails.
