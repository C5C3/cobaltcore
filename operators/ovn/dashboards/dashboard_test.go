...
// against the metrics contract exposed by the OVN operator.
//
...
import (
	...
	"github.com/c5c3/cobaltcore/operators/ovn/internal/metrics"
...
// directory.
const dashboardPath = "ovn-operator.json"
...
// matches down to those starting with `ovn_operator_` so we ignore
...
func TestDashboardParsesAsJSON(t *testing.T) {
	...
	g.Expect(d.UID).To(Equal("ovn-operator"),
	...
	// The backup panel surfaces the per-CR ovn_operator_backup_* collectors,
	// the only view on the database backups the CronJob writes; assert it by
	// title so a rename or accidental removal fails here.
	g.Expect(titles).To(HaveKey("backup duration p95 and failure rate"),
		"dashboard must include the backup duration/failure panel")
...
// drift: every `ovn_operator_*` identifier that appears in a panel target
// must map to a metric the operator actually registers on
...
func TestDashboardReferencesOnlyRegisteredMetrics(t *testing.T) {
	...
	// this test binary is the only ovn_operator registrant, so the mirrored
	// instance yields the exact metric families production emits.
	const probe = "dashboard_test_probe"
	subReconcilerMetrics := instrumentation.NewMetrics("ovn_operator")
	...
	metrics.RecordBackup(probe, probe, "succeeded", 0)
	...
	for _, fam := range families {
		name := fam.GetName()
		if strings.HasPrefix(name, "ovn_operator_") {
			registered[name] = struct{}{}
		}
	}
	// Clean up the probe series so subsequent tests in the same binary see a
	// clean per-CR registry.
	metrics.DeleteForOVNCentral(probe, probe)
	g.Expect(registered).NotTo(BeEmpty(),
		"expected at least one ovn_operator_* metric to be registered")
	...
	for _, p := range d.Panels {
		for _, tr := range p.Targets {
			for _, tok := range metricNameRE.FindAllString(tr.Expr, -1) {
				if !strings.HasPrefix(tok, "ovn_operator_") {
					continue
				}
	...
	g.Expect(seen).NotTo(BeEmpty(),
		"dashboard must reference at least one ovn_operator_* metric")
