...
// Tests for the OVN operator entrypoint wiring. They assert against the
...
// run in this binary and share its one scheme; the OVNCentral list is what pins
// certmanagerv1.AddToScheme, because Certificate is the only kind outside
// client-go on either list.
func TestRemoteChildKindsAreRegisteredInTheScheme(t *testing.T) {
	for name, kinds := range map[string][]schema.GroupVersionKind{
		"OVNCentral": controller.OVNCentralRemoteChildKinds,
		"OVNChassis": controller.OVNChassisRemoteChildKinds,
	} {
