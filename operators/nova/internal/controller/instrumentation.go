...
// Sub-reconciler instrumentation helper for the Nova controller.
package controller
...
//
// "DBConnectionSecrets", "TransportURLSecret" and "Config" deliberately reuse
...
var subReconcilerConditionTypes = map[string]string{
	...
	"DBConnectionSecrets": "SecretsReady",
	"TransportURLSecret":  "SecretsReady",
	"Config":              "SecretsReady",
	"ComputeConfig":       conditionTypeComputeConfigReady,
	"Database":            "DatabaseReady",
	"Conductor":           "ConductorReady",
	"Scheduler":           "SchedulerReady",
	"Metadata":            "MetadataReady",
	"ConsoleProxy":        "ConsoleProxyReady",
	"Deployment":          "DeploymentReady",
	"DBArchive":           conditionTypeDBArchiveReady,
	"HTTPRoute":           conditionTypeHTTPRouteReady,
	"MetadataHTTPRoute":   conditionTypeMetadataHTTPRouteReady,
...
// instrumentation (nova_operator_reconcile_duration_seconds and
// nova_operator_reconcile_errors_total). It owns its metric vectors, which
...
// it.
var instrumenter = instrumentation.NewSubReconcilerInstrumenter("nova_operator", subReconcilerConditionTypes)
...
// and the per-CR db-sync and db-archive collectors. It returns an error on a
...
func RegisterMetrics() error {
	if err := instrumenter.Register(ctrlmetrics.Registry); err != nil {
		return fmt.Errorf("registering nova_operator sub-reconciler metrics: %w", err)
	}
	if err := metrics.Register(); err != nil {
		return fmt.Errorf("registering nova_operator collectors: %w", err)
	}
	return nil
}
