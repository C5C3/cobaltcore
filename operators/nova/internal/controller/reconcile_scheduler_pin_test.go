...
// Byte-identity pin for the scheduler Deployment. The goldens below are
...
// silent pod-template churn that rolls the scheduler on an operator upgrade.
package controller
...
// pinNovaSchedulerDeploymentDefaultGolden is the scheduler Deployment of the
// shared fixture.
const pinNovaSchedulerDeploymentDefaultGolden = `metadata:
  labels:
    ...
    spec:
      ...
      - command:
        ...
        - mountPath: /etc/nova/nova.conf.d
          name: config
          readOnly: true
        - mountPath: /etc/nova/scheduler.conf.d
          name: scheduler-overlay
          readOnly: true
...
// pinNovaSchedulerDeploymentTLSGolden is the same Deployment for a Nova that
// verifies every transport: both schemas' client keypairs and the broker's CA
// bundle.
const pinNovaSchedulerDeploymentTLSGolden = `metadata:
  labels:
    app.kubernetes.io/component: scheduler
    app.kubernetes.io/instance: nova
    app.kubernetes.io/managed-by: nova-operator
...
// TestPinNovaSchedulerDeployment pins what separates the scheduler from the
// front ends: two config directories, the readiness probe taken off the message
// bus, and no HTTP surface at all. The fixture digests go in because the hash
// annotations are the only thing that rolls a running scheduler once the
...
func TestPinNovaSchedulerDeployment(t *testing.T) {
	cases := []struct {
		name   string
		nova   func() *novav1alpha1.Nova
		golden string
	}{
		{name: "default", nova: validNova, golden: pinNovaSchedulerDeploymentDefaultGolden},
		{name: "tls", nova: tlsNova, golden: pinNovaSchedulerDeploymentTLSGolden},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			expectGolden(t, renderYAML(t, buildSchedulerDeployment(tc.nova(), workloadArtifacts(),
				workloadTestDigests(), testEgressPort)), tc.golden)
			expectGolden(t, renderYAML(t, buildSchedulerDeployment(atRelease(tc.nova(), "2026.1"), workloadArtifacts(),
