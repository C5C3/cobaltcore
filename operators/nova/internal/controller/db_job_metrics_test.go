...
func terminalSyncJob(uid string, conditionType batchv1.JobConditionType) *batchv1.Job {
	return &batchv1.Job{
		ObjectMeta: metav1.ObjectMeta{
			Name:      testNovaName + "-" + dbSyncJobSuffix,
...
func TestRecordDBJobTerminalState(t *testing.T) {
	cases := []struct {
		...
		{name: "a failed Job", observed: terminalSyncJob("failed-uid", batchv1.JobFailed), stamped: true},
		{
			...
			observed: &batchv1.Job{ObjectMeta: metav1.ObjectMeta{Name: "nova-db-sync", UID: "running-uid"}},
		},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			r.recordDBJobTerminalState(context.Background(), nova, dbSyncJobSuffix, tc.observed)

			if !tc.stamped {
				g.Expect(nova.Annotations).NotTo(HaveKey(dbJobUIDAnnotationKey(dbSyncJobSuffix)))
				return
			}
			g.Expect(nova.Annotations).To(HaveKeyWithValue(
				dbJobUIDAnnotationKey(dbSyncJobSuffix), string(tc.observed.UID)))
		})
	}
	...
	t.Run("the phases dedupe independently", func(t *testing.T) {
		...
		r.recordDBJobTerminalState(context.Background(), nova, upgradeExpandJobSuffix, expand)
		r.recordDBJobTerminalState(context.Background(), nova, upgradeContractJobSuffix,
...
func TestRecordDBJobTerminalState_PatchFailureDefers(t *testing.T) {
	...
	r.recordDBJobTerminalState(context.Background(), nova, dbSyncJobSuffix,
