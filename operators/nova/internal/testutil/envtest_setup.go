...
// Package testutil provides Nova-specific test utilities for envtest integration tests.
package testutil
...
// SetupNovaEnvTest starts an envtest API server with the Nova CRD installed, the
// webhook server configured and running, and the caller-registered
...
//
// Parameters:
//   - addToScheme registers the Nova API types with the runtime scheme. Callers
//     pass novav1alpha1.AddToScheme to avoid an import cycle between the testutil
//     package and the v1alpha1 package.
//   - registerWebhooks sets up the webhook handler with the manager. The webhook
//     manifests installed by envtest carry the Nova entries with
...
// SetupNovaEnvTestNoWebhook starts an envtest API server with only the Nova CRD
// installed: no webhook configurations, no webhooks. It returns a direct
// controller-runtime client so tests can submit CRs and observe exactly the
// schema-layer validation the API server enforces (kubebuilder validation
// markers + x-kubernetes-validations CEL rules) without the defense-in-depth
// webhook short-circuiting the rejection or filling defaults. Tear-down is wired
// via t.Cleanup().
//
// This is intended for tests that must attribute a rejection to the CRD layer
// alone: the targetClusterRef transition rules, the two-schema rules on
// spec.apiDatabase and spec.database, and the console-proxy pairing rules, all of
// which the validating webhook also checks. If a CEL rule were dropped, the
// equivalent SetupNovaEnvTest-based test could silently keep passing because the
// webhook would still reject the CR; using this helper makes the CRD-layer rule
// the only enforcement point in scope.
func SetupNovaEnvTestNoWebhook(
	t testing.TB,
	addToScheme func(*k8sruntime.Scheme) error,
) (client.Client, context.Context, context.CancelFunc) {
	t.Helper()
...
// SetupNovaEnvTestWithController starts an envtest API server with the Nova CRD,
// webhook configurations, fake CRDs for external operators (MariaDB, ESO,
// Gateway API, ...), and a controller-runtime Manager hosting the
...
//
// Parameters:
//   - addToScheme registers the Nova API types with the runtime scheme.
//   - registerWebhooks sets up the webhook handler with the manager.
//   - registerController wires the NovaReconciler onto the manager. Callers pass
//     controller options with SkipNameValidation set, because controller-runtime
//     rejects a second controller of the same name within one process and every
//     test in a package starts its own manager.
func SetupNovaEnvTestWithController(
	t testing.TB,
	addToScheme func(*k8sruntime.Scheme) error,
	registerWebhooks func(ctrl.Manager) error,
	...
	// Combine the Nova CRD dir with the common fake CRD dirs (ESO, gateway-api,
	// mariadb, ...) so the reconciler's external kinds resolve.
...
// novaPaths returns absolute paths to the Nova CRD and webhook configuration
// directories, resolved relative to this source file via runtime.Caller(0).
func novaPaths() (crdDir, webhookDir string) {
