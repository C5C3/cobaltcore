...
var scheme = bootstrap.NewScheme(
	...
	// the Database/User/Grant CRs of both schemas and watch the MariaDB clusters.
...
func main() {
	if err := bootstrap.Run(bootstrap.ManagerConfig{
		Scheme:           scheme,
		LeaderElectionID: "nova.openstack.c5c3.io",
		// The reconciler resolves spec.targetClusterRef, so the binary engages
