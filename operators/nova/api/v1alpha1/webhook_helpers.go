...
// nova.conf as something other than one option: an empty section name or an
...
// mounts, a connection string or address that points a process somewhere the
// operator did not provision, or a switch that selects a security control.
// registry selects the kind's own list.
...
// exemption is what keeps the keystoneauth credentials out of the scan, since
// keystonemiddleware and the auth plugins register them at runtime and no
// catalog enumerates them. Every remaining unknown option or unknown section
// becomes a field error, and every deprecated-but-accepted option a warning
	...
	for _, u := range unknown {
		...
		if u.SectionUnknown {
			errs = append(errs, field.Invalid(fldPath, u.Key,
				fmt.Sprintf("no such section in the nova %s option catalog", base)))
			continue
		}
		errs = append(errs, field.Invalid(fldPath, u.Key,
			fmt.Sprintf("no such option in the nova %s option catalog", base)))
	}
	...
	for _, d := range deprecated {
		if d.Replacement == "" {
			warnings = append(warnings, fmt.Sprintf(
				"spec.extraConfig [%s] %s: deprecated option in nova %s with no replacement",
				d.Section, d.Key, base,
			))
			continue
		}
		warnings = append(warnings, fmt.Sprintf(
			"spec.extraConfig [%s] %s: deprecated option in nova %s, replaced by %s",
