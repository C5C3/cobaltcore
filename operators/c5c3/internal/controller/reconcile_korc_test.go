...
func TestForceSyncKORCCloudsYAMLExternalSecret_RetriesOnConflict(t *testing.T) {
	...
	err := resyncExternalSecret(context.Background(), r.Client, childNamespace(cp), korcCloudsYamlSecretName, "hash-1")
...
func TestForceRepushAdminAppCredential_StampsHashRetriesAndIdempotent(t *testing.T) {
	...
	g.Expect(repushPushSecret(context.Background(), r.Client, childNamespace(cp), psName,
