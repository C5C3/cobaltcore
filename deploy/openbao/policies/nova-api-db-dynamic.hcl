...
# nova-api-db-dynamic policy: grants read on the per-tenant dynamic MariaDB
# database-engine credentials path for the Nova API database user. The schema it
...
# resolves, at request time, to the namespace of the nova-api-db-creds SA token
# that authenticated. Per-tenant roles are named nova-api-<namespace> (one
...
# can read only database/mariadb/creds/nova-api-A and cannot read another
# tenant's nova-api-B path. The nova-api-db role keeps
...
#
# SERVICE ISOLATION: this policy and its nova-api-db role are entirely separate
# from the keystone-db and cinder-db pairs, and from the nova-cell-db pair that
# covers the same service's cell database. Every one of those roles binds
# bound_service_account_namespaces="*", and they are told apart by the
# ServiceAccount NAME the token carries: nova-api-db-creds here,
# nova-cell-db-creds for nova-cell-db-dynamic, keystone-db-creds for
# keystone-db-dynamic. The API credential generator can therefore never read the
# cell creds path, and the cell generator can never read this one, even though
# both run in one namespace. The two role names are prefix-free on top of that:
# the per-tenant engine role is <role>-<namespace>, so a role named nova in
# namespace api-x and a role named nova-api in namespace x would both flatten to
# nova-api-x. Naming the cell role nova-cell keeps either creds path from
# equalling the other's in any namespace.
#
...
path "database/mariadb/creds/nova-api-{{identity.entity.aliases.KUBERNETES_MANAGEMENT_ACCESSOR.metadata.service_account_namespace}}" {
  capabilities = ["read"]
}
