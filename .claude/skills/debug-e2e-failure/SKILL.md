...
| `e2e-chaos` (`pod` / `network`) | explicit `test_dirs` matrix over `tests/e2e-chaos/`, chaos config (`parallel: 1`, `assert: 300s`); `network` leg is `continue-on-error` (kernel-module dependency stays flaky); all legs run on `self-hosted` | diag dump + `e2e-chaos-junit-report-<suite>` |
| `e2e-prometheus` | `tests/e2e/keystone/prometheus-stack/` with `WITH_PROMETHEUS=true` | diag dump + `e2e-prometheus-junit-report` |
| `e2e-controlplane` | three chainsaw steps on one cluster: `tests/e2e/c5c3/full-controlplane-keystone/` — full chain (c5c3-operator + K-ORC + keystone-operator) — then `tests/e2e/c5c3/keystone-service-foreign-namespace/` (cross-namespace registration, its own Keystone-only ControlPlane, JUnit under `chainsaw-report-keystone-service`), then `tests/e2e/c5c3/keystone-service/` (own-namespace registration, JUnit under `chainsaw-report-keystone-service-own-namespace`); `E2E_REQUIRE_CONTROLPLANE_STACK=true` flips presence-guard SKIPs into failures on all three | diag dump (`OPERATOR=c5c3`, dumps the `openstack` namespace only — the registration suites emit their own evidence in their catch blocks) + `e2e-controlplane-junit-report` |
...
| `ECONNREFUSED` on the API Service correlating with a maintenance CronJob firing (default trust flush: top of the hour) — a `*-trust-flush-*`/`*-db-purge-*` pod shows in the catch dump at the failure timestamp | the maintenance pod's template labels satisfied the API Service selector, so it became an endpoint with nothing listening (numeric `targetPort`, no readiness probe) — a real product bug, not a flake | `1e734cc0` — `component=api` on API pods + two-phase selector narrowing; `tests/e2e/keystone/maintenance-endpoint-isolation` pins the isolation |
