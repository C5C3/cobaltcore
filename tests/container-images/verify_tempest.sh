...
test_keystone_tempest_plugin_importable() {
  ...
  local exit_code=0
  docker run --rm "$IMAGE" python3 -c 'import keystone_tempest_plugin' 2>&1 || exit_code=$?
...
test_subunit2junitxml_available() {
  ...
  path=$(docker run --rm "$IMAGE" which subunit2junitxml 2>&1) || exit_code=$?

  assert_eq "which subunit2junitxml exits 0" "0" "$exit_code"
  assert_not_empty "subunit2junitxml path is non-empty" "$path"
}
