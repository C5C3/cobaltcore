...
test_every_filter_steers_something() {
  ...
  local exempt=" publish_legacy tempest_keystone tempest_glance tempest_barbican tempest_neutron tempest_cinder tempest_nova "
  ...
  assert_contains "publish_legacy fills the publish matrix on a push" \
    ...
    'e2e-operators={"operator":["keystone","c5c3","horizon","glance","placement","barbican","ovn","neutron","cinder","nova"]}'
  ...
  local svc
  for svc in keystone glance barbican neutron cinder nova; do
