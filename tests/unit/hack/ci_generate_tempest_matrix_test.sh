...
make_tree() {
  ...
  for release in "$@"; do
    ...
    for service in keystone glance barbican neutron cinder; do
...
test_unset_selection_emits_every_service() {
  ...
  assert_eq "every service is emitted for every release" \
    "barbican barbican cinder cinder glance glance keystone keystone neutron neutron" "$(services_in_matrix)"
