...
STUB
  cat >"$dir/kind" <<'STUB'
#!/bin/bash
echo "kind v0.32.0 go1.23.0 linux/amd64"
STUB
  cat >"$dir/kubectl" <<'STUB'
#!/bin/bash
echo "Client Version: v1.36.2"
STUB
...
test_default_skips_install_flux() {
  ...
  assert_not_contains "no 'flux ... already installed' log line" "$output" "flux 2.9.2 already installed"
  # Other installers still run (they short-circuit on the pre-populated stubs).
  assert_contains "install_chainsaw still runs" "$output" "chainsaw v0.2.15 already installed"
  assert_contains "install_kind still runs" "$output" "kind v0.32.0 already installed"
  assert_contains "install_kubectl still runs" "$output" "kubectl v1.36.2 already installed"
...
test_with_flux_cli_true_invokes_install_flux() {
  ...
  assert_contains "install_flux was invoked" "$output" "Installing flux 2.9.2"
  ...
  assert_contains "install_flux completed" "$output" "flux 2.9.2 installed to"
...
test_with_flux_cli_true_short_circuits_on_correct_version() {
  ...
  assert_contains "short-circuit log line appears" "$output" "flux 2.9.2 already installed"
  # The download + verify branches must NOT have run.
  assert_not_contains "no 'Installing flux' log line" "$output" "Installing flux 2.9.2"
