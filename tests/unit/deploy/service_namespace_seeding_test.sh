...
#                         <neutron_declared> <neutron_ns> <neutron_dedicated_db> \
#                         <cinder_declared> <cinder_ns> <cinder_dedicated_db>
# A kubectl stub that answers EVERY lookup setup-database-tenant.sh makes so the
# keystone, glance, placement, barbican, neutron AND cinder legs all run to
# completion — the bao writes reach OpenBao via `kubectl exec ... openbao-0`,
# which the stub swallows.
#
# <glance_declared> / <placement_declared> / <barbican_declared> /
# <neutron_declared> / <cinder_declared> are the presence gate the script reads
...
# <neutron_dedicated_db> / <cinder_dedicated_db> declare a dedicated database for
# that service when non-empty (Static-only, so no engine role). Keystone stays
...
make_kubectl_full() {
  ...
  local neutron_declared="${11}" neutron_ns="${12}" neutron_dedicated_db="${13}"
  local cinder_declared="${14}" cinder_ns="${15}" cinder_dedicated_db="${16}"
  mkdir -p "$dir"
  cat >"$dir/kubectl" <<STUB
#!/bin/bash
...
if [[ "\$*" == *"jsonpath={.spec.services.neutron.dedicatedBackingServices.database}"* ]]; then
  printf '%s' "${neutron_dedicated_db}"
  exit 0
fi
if [[ "\$*" == *"jsonpath={.spec.services.neutron.namespace.name}"* ]]; then
  printf '%s' "${neutron_ns}"
  exit 0
fi
if [[ "\$*" == *"jsonpath={.spec.services.neutron}"* ]]; then
  # A non-empty object marks Neutron as declared; its exact shape is irrelevant.
  # An undeclared Neutron prints nothing, which the script's -z gate reads.
  if [[ -n "${neutron_declared}" ]]; then
    printf 'map[namespace:map[name:%s]]' "${neutron_ns}"
  fi
  exit 0
fi
if [[ "\$*" == *"jsonpath={.spec.services.cinder.dedicatedBackingServices.database}"* ]]; then
  printf '%s' "${cinder_dedicated_db}"
  exit 0
fi
if [[ "\$*" == *"jsonpath={.spec.services.cinder.namespace.name}"* ]]; then
  printf '%s' "${cinder_ns}"
  exit 0
fi
if [[ "\$*" == *"jsonpath={.spec.services.cinder}"* ]]; then
  # A non-empty object marks Cinder as declared; its exact shape is irrelevant.
  # An undeclared Cinder prints nothing, which the script's -z gate reads.
  if [[ -n "${cinder_declared}" ]]; then
    printf 'map[namespace:map[name:%s]]' "${cinder_ns}"
  fi
  exit 0
fi
if [[ "\$*" == *"jsonpath={.spec.infrastructure.database.clusterRef.name}"* ]]; then
...
# ---------------------------------------------------------------------------
# Test: the neutron engine pair follows the Neutron service namespace
# ---------------------------------------------------------------------------
test_db_tenant_provisions_neutron_on_the_neutron_namespace() {
  echo "Test: setup-database-tenant.sh keys the neutron engine pair on the Neutron namespace"

  local tmp
  tmp="$(mktemp -d)"
  trap 'rm -rf "$tmp"' RETURN

  # Neutron declared, placed in its own "networking" namespace, no dedicated database.
  make_kubectl_full "$tmp" "yes" "images" "" "yes" "compute" "" "yes" "secrets" "" "yes" "networking" "" "" "" ""

  local output
  output="$(run_db_tenant "$tmp" "openstack" "cp")"

  assert_contains "the neutron engine role is keyed on the Neutron service namespace" \
    "$output" "database/mariadb/roles/neutron-networking"
  assert_contains "the neutron connection config is keyed on the same namespace" \
    "$output" "database/mariadb/config/neutron-networking"
  assert_contains "the neutron role issues users on the fixed 'neutron' schema" \
    "$output" "Database  : neutron"
  assert_contains "the neutron MariaDB is read from the Neutron service namespace" \
    "$output" "openstack-db.networking.svc:3306"
  assert_contains "keystone is still provisioned on the ControlPlane namespace" \
    "$output" "database/mariadb/roles/keystone-openstack"
  assert_not_contains "the neutron role must not be keyed on the ControlPlane namespace" \
    "$output" "roles/neutron-openstack"
}

# ---------------------------------------------------------------------------
# Test: a dedicated neutron database skips the neutron engine leg
# ---------------------------------------------------------------------------
test_db_tenant_skips_neutron_with_a_dedicated_database() {
  echo "Test: setup-database-tenant.sh skips the neutron leg for a dedicated neutron database"

  local tmp
  tmp="$(mktemp -d)"
  trap 'rm -rf "$tmp"' RETURN

  # Neutron declared WITH a dedicated database — Static-only, so no engine role.
  make_kubectl_full "$tmp" "yes" "images" "" "yes" "compute" "" "yes" "secrets" "" "yes" "networking" "neutron-db" "" "" ""

  local output
  output="$(run_db_tenant "$tmp" "openstack" "cp")"

  assert_contains "the dedicated-database neutron leg is skipped, loudly" \
    "$output" "ControlPlane declares a dedicated neutron database"
  assert_contains "the skip names the neutron database-engine tenant" \
    "$output" "skipping the neutron database-engine tenant"
  assert_not_contains "no neutron engine role is written for a dedicated database" \
...
# ---------------------------------------------------------------------------
# Test: the cinder engine pair follows the Cinder service namespace
# ---------------------------------------------------------------------------
test_db_tenant_provisions_cinder_on_the_cinder_namespace() {
  echo "Test: setup-database-tenant.sh keys the cinder engine pair on the Cinder namespace"

  local tmp
  tmp="$(mktemp -d)"
  trap 'rm -rf "$tmp"' RETURN

  # Cinder declared, placed in its own "storage" namespace, no dedicated database.
  make_kubectl_full "$tmp" "yes" "images" "" "yes" "compute" "" "yes" "secrets" "" "yes" "networking" "" "yes" "storage" ""

  local output
  output="$(run_db_tenant "$tmp" "openstack" "cp")"

  assert_contains "the cinder engine role is keyed on the Cinder service namespace" \
    "$output" "database/mariadb/roles/cinder-storage"
  assert_contains "the cinder connection config is keyed on the same namespace" \
    "$output" "database/mariadb/config/cinder-storage"
  assert_contains "the cinder role issues users on the fixed 'cinder' schema" \
    "$output" "Database  : cinder"
  assert_contains "the cinder MariaDB is read from the Cinder service namespace" \
    "$output" "openstack-db.storage.svc:3306"
  assert_contains "keystone is still provisioned on the ControlPlane namespace" \
    "$output" "database/mariadb/roles/keystone-openstack"
  assert_not_contains "the cinder role must not be keyed on the ControlPlane namespace" \
    "$output" "roles/cinder-openstack"
}

# ---------------------------------------------------------------------------
# Test: a dedicated cinder database skips the cinder engine leg
# ---------------------------------------------------------------------------
test_db_tenant_skips_cinder_with_a_dedicated_database() {
  echo "Test: setup-database-tenant.sh skips the cinder leg for a dedicated cinder database"

  local tmp
  tmp="$(mktemp -d)"
  trap 'rm -rf "$tmp"' RETURN

  # Cinder declared WITH a dedicated database — Static-only, so no engine role.
  make_kubectl_full "$tmp" "yes" "images" "" "yes" "compute" "" "yes" "secrets" "" "yes" "networking" "" "yes" "storage" "cinder-db"

  local output
  output="$(run_db_tenant "$tmp" "openstack" "cp")"

  assert_contains "the dedicated-database cinder leg is skipped, loudly" \
    "$output" "ControlPlane declares a dedicated cinder database"
  assert_contains "the skip names the cinder database-engine tenant" \
    "$output" "skipping the cinder database-engine tenant"
  assert_not_contains "no cinder engine role is written for a dedicated database" \
...
test_db_tenant_skips_undeclared_services() {
  ...
  assert_contains "the undeclared neutron leg is skipped, loudly" \
  ...
  assert_not_contains "no barbican engine role is written for an undeclared service" \
    "$output" "database/mariadb/roles/barbican-"
  assert_not_contains "no neutron engine role is written for an undeclared service" \
    "$output" "database/mariadb/roles/neutron-"
