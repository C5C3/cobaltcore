...
"""Fast unit tests for the invalid-NovaCompute fixture generator.
...
def _load_generator() -> types.ModuleType:
    spec = importlib.util.spec_from_file_location("nova_invalid_novacompute_cr_generate", _GENERATOR)
...
class TestFixtures(unittest.TestCase):
    ...
    def test_scaffold_name_within_webhook_bound(self) -> None:
        """Only the long-name fixture may exceed MaxNovaComputeNameLength (63).
        ...
        """
        for fixture in self.generator.FIXTURES:
            if fixture.filename == "02-name-too-long.yaml":
                self.assertEqual(len(fixture.name), self.generator.MAX_NAME_LENGTH + 1)
                continue
            self.assertLessEqual(
                len(fixture.name),
                self.generator.MAX_NAME_LENGTH,
                f"{fixture.filename} name exceeds MaxNovaComputeNameLength",
            )
