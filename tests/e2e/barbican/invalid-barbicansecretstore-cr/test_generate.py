...
"""Fast unit tests for the invalid BarbicanSecretStore fixture generator.

Mirrors tests/e2e/barbican/invalid-cr/test_generate.py: guards the
...
# Number of fixtures emitted by _generate.py: twelve create-rejection fixtures
# (00-11), three immutability pairs (12/13 barbicanRef, 14/15 kvMountpoint,
# 16/17 store mode), and two sibling pairs (18/19 single default, 20/21 OpenBao
# uniqueness). Bumping this value requires adding the matching Fixture entry AND
# the matching `file: <name>` line in chainsaw-test.yaml.
_EXPECTED_FIXTURE_COUNT = 22


def _load_generator() -> types.ModuleType:
    spec = importlib.util.spec_from_file_location("barbicansecretstore_invalid_cr_generate", _GENERATOR)
...
class TestFixtures(unittest.TestCase):
    ...
    def test_transition_pairs_share_a_name(self) -> None:
        """Each pair's update fixture must reuse its base CR's metadata.name.

        A renamed update half would be applied as a CREATE, which the
        UPDATE-only transition rules never see — the step would then fail on an
        admitted CR instead of pinning the rule.
        """
        by_filename = {fixture.filename: fixture for fixture in self.generator.FIXTURES}
        pairs = (
            ("12-immutable-barbicanref-base.yaml", "13-immutable-barbicanref.yaml"),
            ("14-immutable-kvmountpoint-base.yaml", "15-immutable-kvmountpoint.yaml"),
            ("16-immutable-mode-base.yaml", "17-immutable-mode.yaml"),
        )
        for base, update in pairs:
            self.assertEqual(by_filename[base].name, by_filename[update].name, f"{update} must reuse the name of {base}")
