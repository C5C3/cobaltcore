...
# Recover the root token from the init-output Secret (mirrors seal-all.sh:51).
# The same Secret feeds both seal/unseal and policy admin commands.
read_root_token() {
  local init_output
...
main() {
  ...
  local policy_text
  if ! policy_text=$(kubectl exec -n "${BAO_NAMESPACE}" "${BAO_POD}" -- \
