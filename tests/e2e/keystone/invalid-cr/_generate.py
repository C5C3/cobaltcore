...
# the two hand-written fixtures are exempted via _HAND_WRITTEN_FIXTURES so
# the sweep does not flag them as orphans.
_FIXTURE_FILENAME_PATTERN = re.compile(r"^[0-9]{2}-.+\.yaml$")

# filenames that match _FIXTURE_FILENAME_PATTERN but predate this generator
# and are intentionally NOT regenerated by it. The orphan sweep in main() must
# skip these so it does not delete or flag them as drift.
_HAND_WRITTEN_FIXTURES = frozenset(
    {
...
def main(argv: list[str]) -> int:
    ...
    # Files in _HAND_WRITTEN_FIXTURES are exempted because they predate this
    # generator and are not generated here.
    expected = {f.filename for f in FIXTURES} | _HAND_WRITTEN_FIXTURES
