...
"""Fast unit tests for the invalid-OVNChassis fixture generator.

Mirrors tests/e2e/ovn/invalid-cr/test_generate.py: guards the
...
# chainsaw-test.yaml.
_EXPECTED_FIXTURE_COUNT = 15


def _load_generator() -> types.ModuleType:
    spec = importlib.util.spec_from_file_location("ovn_invalid_ovnchassis_cr_generate", _GENERATOR)
...
class TestFixtures(unittest.TestCase):
    ...
    def test_scaffold_name_within_webhook_bound(self) -> None:
        """Only the long-name fixture may exceed MaxOVNChassisNameLength (42).
        ...
        """
        for fixture in self.generator.FIXTURES:
            if fixture.filename == "10-name-too-long.yaml":
                self.assertGreater(len(fixture.name), 42)
                continue
            self.assertLessEqual(
                len(fixture.name),
                42,
                f"{fixture.filename} name exceeds MaxOVNChassisNameLength",
            )
