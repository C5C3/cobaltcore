...
"""Fast unit tests for the invalid-NeutronMetadataAgent fixture generator.

Mirrors tests/e2e/ovn/invalid-ovnchassis-cr/test_generate.py: guards the
...
* The long-name fixture sits exactly one character past
  MaxNeutronMetadataAgentNameLength and every other fixture clears the bound.
...
# chainsaw-test.yaml.
_EXPECTED_FIXTURE_COUNT = 12

# MaxNeutronMetadataAgentNameLength in
# operators/neutron/api/v1alpha1/neutronmetadataagent_types.go: the cap
# Kubernetes puts on the label value the name becomes on every child.
_MAX_NAME_LENGTH = 63
_LONG_NAME_FIXTURE = "08-name-too-long.yaml"


def _load_generator() -> types.ModuleType:
    spec = importlib.util.spec_from_file_location("neutron_invalid_neutronmetadataagent_cr_generate", _GENERATOR)
...
class TestFixtures(unittest.TestCase):
    ...
    def test_scaffold_name_within_webhook_bound(self) -> None:
        """Only the long-name fixture may exceed MaxNeutronMetadataAgentNameLength (63).
        ...
        for fixture in self.generator.FIXTURES:
            ...
            self.assertLessEqual(
                len(fixture.name),
                _MAX_NAME_LENGTH,
                f"{fixture.filename} name exceeds MaxNeutronMetadataAgentNameLength",
            )
