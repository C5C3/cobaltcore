...
test_glance_test_matrices() {
  ...
  assert_contains \
    "a glance change puts glance in the test matrix" \
...
test_glance_helm_validate_loops() {
  ...
  [ -d "operators/glance/helm/glance-operator" ] || loop_count=0
...
test_resolve_script_documents_filter() {
  echo "Test: FILTER_glance reaches the resolve script"

  # The resolver reads the per-operator filters through ALL_OPERATORS rather
  # than naming each one, so asserting the literal FILTER_glance against its
  ...
  assert_file_contains \
    "ci.yaml passes FILTER_glance to the resolve step" \
    "$CI_YAML" \
    "FILTER_glance: \${{ steps.filter.outputs.glance }}"

  assert_file_contains \
    "ci.yaml lists glance in ALL_OPERATORS" \
    "$CI_YAML" \
    "ALL_OPERATORS: .*glance"
